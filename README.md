...
You really did need to have a cron job to run Skulker every once in a whike. You could change the settings to not touch tuff (like in RAM based /tmp storage where often running applicaionb files were stored"
 ...
 1. Deletes files not touchedf in a set period of time
  2 Clenas up authorized /tmp files
    3. Cleams duplicates\
    4. etc/

In other owrds. You used to have to program your computer to do stuff like clean up. Now it does it for you, and it always had, just required a system programmer to go write cron job. There is something nostalgic about building one for yourself in a langauge you lke (Go). Zig reminds me a ot of
C. Not in content, but in attitude. Thts a topic for another essay.
